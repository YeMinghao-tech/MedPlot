...
| 任务编号 | 任务名称 | 状态 | 备注 |
|---------|---------|------|------|
| F1 | FastAPI 应用入口（app.py） | [x] | 注册路由、中间件、生命周期管理 |
| F2 | 会话管理路由（session.py） | [x] | 创建/获取/删除会话，session_id 生成 |
| F3 | 对话接口（chat.py） | [x] | POST /chat，WebSocket 支持 |
| F4 | 患者档案接口（patient.py） | [x] | 查询/更新患者信息（脱敏） |
| F5 | 认证中间件（auth.py） | [x] | JWT 验证，患者身份绑定 |
| F6 | 主链路联调 | [x] | 模拟患者从”问诊→查号→挂号确认”全流程 |
...
| 任务编号 | 任务名称 | 状态 | 备注 |
|---------|---------|------|------|
| K1 | 黄金测试集构建（Golden Test Set） | [x] | 医疗知识问答、症状到科室、病历生成、挂号场景 |
| K2 | RagasEvaluator 实现 | [x] | Faithfulness、Answer Relevancy、Context Precision |
| K3 | CompositeEvaluator 实现 | [x] | 多评估器并行，结果汇总 |
| K4 | EvalRunner 实现 | [x] | 加载黄金测试集，输出指标报告 |
| K5 | LLM-as-Judge 幻觉检测 | [x] | 使用强模型计算 Faithfulness，阈值 ≥ 0.95 |
| K6 | CI 自动化门禁 | [x] | PR 阶段自动运行评估，低于阈值阻断 |
| K7 | Release 人工门禁 | [x] | 提取低置信度 Edge Cases，人工签署 |
| K8 | 评估面板页面（Evaluation Panel） | [x] | 指标趋势图、历史对比 |
...
| 任务编号 | 任务名称 | 状态 | 备注 |
|---------|---------|------|------|
| L1 | E2E：完整问诊流程 | [x] | 从输入症状到生成病历 |
| L2 | E2E：复诊记忆唤醒流 | [x] | 验证情景记忆检索 |
| L3 | E2E：急症红线拦截 | [x] | 输入”胸痛放射至左肩”，验证强制拦截 |
| L4 | E2E：挂号完整闭环 | [x] | 从科室选择到锁号成功 |
| L5 | E2E：隐私数据隔离 | [x] | 多会话并发，验证数据隔离 |
| L6 | E2E：Dashboard 冒烟测试 | [x] | 所有页面可加载、不抛异常 |
| L7 | 完善 README | [x] | 快速开始、配置说明、API 文档、Dashboard 使用 |
| L8 | 全链路验收 | [x] | `pytest` 全绿 + 手动走通完整流程 |
